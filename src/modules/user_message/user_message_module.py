...
class UserMessageModule(BaseModule):
    ...
    message_tag = "[user:xxx:zzz:yyyy-yy-yy hh:mm:ss]"
    message_tag_instruction = "标签后跟用户消息内容，标签中的xxx表示用户id，zzz表示用户发送这条消息的渠道（如web、qq等，默认值为web），yyyy-yy-yy hh:mm:ss表示用户发送消息的时间"
